// src/pages/ChatDoctor/index.jsx
...
const ChatDoctor = () => {
  ...
  const handleSendMessage = () => {
    ...
    const userMessage = { id: messages.length + 1, type: 'user', text: input, timestamp: new Date() };
    setMessages(prev => [...prev, userMessage]);
    setInput('');
    setLoading(true);
    setTimeout(() => {
      const botMessage = { id: messages.length + 2, type: 'bot', text: getAIResponse(input), timestamp: new Date() };
        ...
        <div className="feature-container">
          ...
          <div className="chat-container">
            <div className="chat-messages">
              ...
              {loading && (
                <div className="chat-message chat-bot">
                  <div className="chat-bubble loading"><span></span><span></span><span></span></div>
                </div>
              )}
            ...
            <div className="chat-input-container">
              ...
              />
              <button onClick={handleSendMessage} disabled={!input.trim() || loading} className="btn btn--primary chat-send-btn">
                {loading ? '...' : 'Send 📤'}
              </button>
            </div>
          ...
          <div className="quick-questions">
            ...
            <div className="quick-questions-grid">
              <button onClick={() => handleQuickQuestion('What can I do to improve my sleep quality?')} className="quick-btn">😴 Sleep Tips</button>
              <button onClick={() => handleQuickQuestion('How to maintain a healthy diet and nutrition?')} className="quick-btn">🥗 Diet Advice</button>
              <button onClick={() => handleQuickQuestion('What exercises should I do for my health?')} className="quick-btn">🏃 Exercise Guide</button>
              <button onClick={() => handleQuickQuestion('How can I manage stress and anxiety?')} className="quick-btn">🧘 Stress Management</button>
              <button onClick={() => handleQuickQuestion('I have a headache, what should I do?')} className="quick-btn">🤕 Headache Relief</button>
              <button onClick={() => handleQuickQuestion('How can I achieve a healthy weight?')} className="quick-btn">⚖️ Weight Management</button>
              <button onClick={() => handleQuickQuestion('I have a fever, what should I do?')} className="quick-btn">🌡️ Fever Help</button>
              <button onClick={() => handleQuickQuestion('How can I manage anxiety symptoms?')} className="quick-btn">💚 Anxiety Support</button>
            </div>
          </div>
        </div>
      </main>
